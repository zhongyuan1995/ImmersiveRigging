...
public:
    ...
    template<class Iter> Rect(Iter start, const Iter &finish) : empty(false)
    {
        ...
        lo = hi = *start;
        while(++start != finish) {
            lo = lo.apply(minimum_func<Real>(), *start);
            hi = hi.apply(maximum_func<Real>(), *start);
        }
    }
    ...
    Self operator&(const Self &other) const
    {
        if(!empty && !other.empty)
            return Self(lo.apply(maximum_func<Real>(), other.lo), hi.apply(minimum_func<Real>(), other.hi));
        return Self();
    }
    ...
    Self operator|(const Self &other) const
    {
        ...
        if(other.empty)
            return (*this);
        return Self(false, lo.apply(minimum_func<Real>(), other.lo), hi.apply(maximum_func<Real>(), other.hi));
    }
